...
class LinkedinEasyApply:
    ...
    def login(self):
        try:
            ...
            log.warning(f"Automated login failed ({type(e).__name__}): could not find login elements.")
            log.info("============================================================")
            log.info("  Please log in manually in the browser window.")
            log.info("  Once you are on the LinkedIn feed, press ENTER here.")
            log.info("============================================================")
    ...
    def additional_questions(self):
        ...
        if len(frm_el) > 0:
            for el in frm_el:
                # Radio check
                try:
                    radios = el.find_element(By.CLASS_NAME,'jobs-easy-apply-form-element').find_elements_by_class_name('fb-radio')
